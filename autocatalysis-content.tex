...
Three representative autocatalytic cycles in central carbon metabolism: (I) The Calvin-Benson-Bassham cycle (yellow); (II) The glyoxylate cycle (magenta); (III) A cycle using the phosphotransferase system (PTS) to assimilate glucose (cyan).
...
\subsubsection{Input flux increases the range of parameters for which a stable steady state solution exists}
  ...
  In these two cases, these shortcuts relax the constraints imposed by strict use of the corresponding autocatalytic cycles as they prevent zero from being a stable steady state concentration.
    ...
    If $Y$ is consumed very slowly, and if the maximal consumption of $Y$ is larger than $V_{\max,a}$, then, as long as $V_{\max,b}>V_{\max,a}$, a stable steady state exists both when  $V_{\max,b}\rightarrow\infty$, making $f_b$ operate near equilibrium, and when $V_{\max,b}\rightarrow V_{\max,a}$.
...
\end{figure}
    ...
    The analysis for a larger number of intermediate metabolites and reactions is given in the methods.
    ...
    Deriving a stability criterion for the multiple-reaction case, we get that in this case a steady state is stable if there exists $i$ such that $\beta_i > \alpha_i$ (where $i$ can now be any number in the range $1\dots n$).
...
\end{figure}
    ...
    We analyze potential allosteric interactions that can support fast convergence and stability of autocatalytic cycles in the methods section.
...
This observation may also highlight the importance of seemingly redundant reactions such as the one catalyzed by rpi, in growth on ribose.
...
\end{enumerate}
  ...
  The simplest equation describing a bisubstrate reaction assumes substituted enzyme (Ping Pong) mechanism \cite{Imperial2014-nw}.
...
\subsection{Extending the two reaction cycle to multiple reaction cycle}
    The two-metabolites cycle case can be easily extended to a larger number of intermediate metabolites and reactions, as is depicted in figure \ref{fig:multiple}B.
...
\begin{equation*}
 ...
 Thus, to keep deviations from steady state at check, it is beneficial to increase $\beta_j$, for all $j$, which implies decreasing the saturation of $f_{b_j}$.
    ...
    Assuming $X^*$ is again the desired steady state, and is attained for some constant concentration of the assimilated metabolite, $A^*$, it follows that because the autocatalytic activity is higher when $A>A^*$, then in order to maintain $X^*$ close to its desired level, when $A>A^*$, $f_a$ should be inhibited, and $f_b$ should be activated, but when $A<A^*$, $f_a$ should be activated, and $f_b$ should be inhibited.
