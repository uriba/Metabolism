...
We note that under the assumption of constant concentration of $A$, the bisubstrate velocity equation for $f_a$ can be reduced to a simple Michaelis-Menten equation the kinetic constants of which depend on the constant value of $A$ assumed (see methods).
We therefore assume that:
    ...
    In the analysis above, to keep the analysis concise, we neglected the bisubstrate nature of the autocatalytic reaction.
    We note that, as is shown in the methods, bisubstrate reactions also take a Michaelis-Menten like form once the concentration of the assimilated metabolite is kept constant.
    Moreover, to attain a stable steady state, the concentration of the assimilated metabolite must exceed a lower threshold, the exact value of which depends on the exact bisubstrate reaction scheme and the kinetic parameters of it, as is shown in the methods.
    On the other hand, if the bisubstrate reaction is such that the internal metabolite $X$ binds first, then no strict constraints exist on the kinetic parameters of the reactions and a stable steady state solution can always be achieved by setting appropriate values to $V_{\max,a}$ and $V_{\max,b}$.
...
\end{enumerate}
  ...
  \end{equation*}
  However, unlike in the previous two cases, in this case if $V_{\max}'<V_{\max,b}$, then for any concentration of $A$ that is larger than its lower bound, the second inequality will be satisfied resulting in a stable steady state.
