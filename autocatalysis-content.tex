...
\subsubsection{Input flux increases the range of parameters for which a stable steady state solution exists}
  ...
  When an input flux is present, an autocatalytic cycle can always achieve a non zero, stable steady state by tuning the expression levels of the enzymes forming the cycle.
    ...
    Deriving stability criteria for the complex case, we get that the characteristic polynomial is:
    \begin{equation}
        \label{eq:complex}
    ...
    \end{equation}
    Using the lemma that relates derivatives to saturation levels (see Methods) we conclude that a sufficient condition for stable autocatalytic fluxes is that the saturation of every branch reaction will be lower than the saturation of the corresponding autocatalytic reaction:
    \begin{equation}
