  ...
  \begin{scope}[shift={(-4cm,4.3cm)}]
        ...
        \draw [->,line width=1pt,autocatacyc] (X.south west) arc (285:-40:1cm) node [pos=0.6,above] (fa) {$f_a:$\small{$E+X\rightarrow2X$}} node [pos=0.45,shape=coordinate] (midauto) {};
        ...
        \begin{customlegend}[legend entries={$f_a$,$V_{\max,b}>V_{\max,a}$,$f_b$,$\sfrac{V_{\max,b}}{V_{\max,a}}<\sfrac{K_{M,b}}{K_{M,a}}$,$\dot{X}=f_a-f_b$},legend style={right=2.5cm of orig,anchor=west,yshift=-3mm,name=legend1,visible on=<3->},legend columns=2]
