...
\begin{tikzpicture}
  ...
  \node[rectangle,draw,below=of pyr.center] (aca) {aca};
  ...
  \node[rectangle,draw,] at(bpg -| xu5p.center) (co2) {co2};
  \draw[->] (g6p) -- (f6p);
  ...
  \node[rectangle,draw,shape=coordinate,left=2.1cm of gluc.center] (ptsbottom) {};
  ...
  \draw[] (co2) [out=-90,in=0] to (rubisco);
...
(B) Three representative autocatalytic cycles in central carbon metabolism: (I) The Calvin Benson Bessahm cycle; (II) The glyoxilate cycle; (III) A cycle using the PTS system to assimilate glucose.
}
\end{figure}
\end{document}
