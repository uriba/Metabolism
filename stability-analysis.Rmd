...
For simplicity, a value of 50 indicates non-feasible solution (this will be corrected in future versions).
...
renderPlot({
  ...
  for(ki in ks)
      for(kb in ks) {
          ...
          x <- fi*Kmi/(Vi-fi)
          if(!is.finite(c)) { rc[ki,kb] <- 50
          } else if(c<0) { rc[ki,kb] <- 50
          } else if(fi<0) {rc[ki,kb] <- 50
          } else if((Vb/(c+Kmb)-Vb*c/((c+Kmb)^2))<(1/(c+1)-1*c/((c+1)^2))) { rc[ki,kb] <- 50
          } else rc[ki,kb] <-x
      }
  par(mar=c(4,6,4,4))
