...
We note that at steady state $f_i = g-f_a$ (and also that $f_p=f_a+g$, thought this is irrelevant at this point), so the higher the auto-catalytic flux, the lower the Xylose input flux, implying a lower concentration at which the phenotype can grow at the specified growth rate.
...
We plot the fitness landscape according to two parameters, $V_{\max,e}$ and $K_{m,b_c}$, the affinity of the biomass reaction to $C$.

Our equations demand that:
$$
f_i+f_a = f_b
f_p=f_e+f_b
f_e = f_a+f_b
...
This extended model includes an intermediate metabolite in the cycle.
We explore how $K_{m,b}$ affects the values of $C_1$ and $C_2$ at the stable steady state in the figure below.

```{r, echo=FALSE}
maxb = 5
inputPanel(
  sliderInput("fi2", label = "$$f_i$$",
              min = 0, max = 1, value = 0.1, step = 0.1),
  sliderInput("Kmc", label = "$$K_{m,c}$$",
              min = 0.1, max = 2, value = 2, step = 0.1),
  sliderInput("Vb2", label = "$$V_{max,b}$$",
              min = 0.1, max = 2, value = 1.5, step = 0.1),
  sliderInput("Vc", label = "$$V_{max,c}$$",
              min = 0.1, max = 2, value = 1.5, step = 0.1)
)

is.stable <- function(x,kb,vb) {
    if(!is.finite(x)) return(FALSE)
    if(x<0) return(FALSE)
    va = 1
    ka = 1
    return((vb/(x+kb)-vb*x/((x+kb)^2))>(va/(x+ka)-va*x/((x+ka)^2)))
}

renderPlot({
  kbs <- seq(0.1,maxb,0.1)
  fi <- input$fi2
  vb <- input$Vb2
  vc <- input$Vc
  kc <- input$Kmc
  ...
  for(kb in kbs) {
      a <- fi + va - vb
      b <- fi*(ka+kb)+va*kb-vb*ka
      c <- fi*ka*kb
      c1 <- (-b+sqrt(b^2-4*a*c))/(2*a)
      c2 <- (-b-sqrt(b^2-4*a*c))/(2*a)
      if(is.stable(c1,kb,vb)) {
          c1s <- append(c1s,c1)
      } else if(is.stable(c2,kb,vb)) {
          c1s <- append(c1s,c2)
      } else {
          c1s <- append(c1s,NaN)
      }
  ...
  data <- data.frame(xs = kbs, c1s = c1s, c2s = c2s)
  data <- melt(data,id=c("xs"))
  size = element_text(size=18)
  ssize = element_text(size=12)
  ggplot(data = data,aes(x=xs,y=value,colour=variable)) + geom_line() +
    xlab(expression(K["m,b"]))+ylab("[C]")+
    scale_color_manual(name="Concentrations",values = c("red","green"),labels=c(expression(c[1]),
                                                                               expression(c[2])))+
    guides(colour= guide_legend())+theme(legend.text=size,legend.title=size,axis.text=ssize,axis.title=size)
})
...
As can be seen, the relation between the concentrations of $C_1$ and $C_2$ changes as $K_{m,b}$ changes, suggesting that, if another biomass reaction exists from $C_2$, it may affect the stability landscape.

We therefore extend our model by adding such a reaction to receive the model depicted below.

#### insert image of "final" model.

This model results in the following dynamics:


```{r, echo=FALSE}
maxb = 5
inputPanel(
  sliderInput("fi3", label = "$$f_i$$",
              min = 0, max = 1, value = 0.1, step = 0.1),
  sliderInput("Kmc2", label = "$$K_{m,c}$$",
              min = 0.1, max = 2, value = 2, step = 0.1),
  sliderInput("Vb3", label = "$$V_{max,b}$$",
              min = 0.1, max = 2, value = 1.5, step = 0.1),
  sliderInput("Vc2", label = "$$V_{max,c}$$",
              min = 0.1, max = 2, value = 1.5, step = 0.1)
)
