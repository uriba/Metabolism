...
Such a setup implies a constant growth rate, $f_b=g$.
...
Stated differently, if two strains, $S_1$ and $S_2$, are both able to grow at rate $g$, but $S_1$ sustains this rate at an outside metabolite concentration $X_1<X_2$, then it follows that at $X_2$, $S_1$ will grow at a faster rate than $S_2$ and will thus out-compete it in the chemostat.

We therefore replace the input flux, $f_i$, with the affinity of the transporter of $X$, $K_{m,i}$.
We assume membrane area and transporter size remain constant and thus $V_{max,i}$ cannot change.
...
Initially, $V_{max,b}=2>V_{max,i}$, implying that even at high concentration of the external metabolite, the biomass reaction is not saturated, meaning high concentrations of $X$ will not cause the cycle to explode.
All of these parameters can be modified below to see their effect on the generated fitness landscape.
...
renderPlot({
  ...
  for(V in vs) 
      for(K in ks) {
          ...
          if(is.stable(x1,Vb,Kmb)) {
              rc[V,K] <- x1
          } else if(is.stable(x2,Vb,Kmb)) {
              rc[V,K] <- x2
          } else rc[V,K] <- 50
      }
  image.plot(x=vs/scale,y=ks/scale,rc,xlab = expression(V[max,b]),ylab = expression(K[m,b]))
})
