...
\author{Uri Barenholz, Dan Davidi, Yinon Bar-On, Ron Milo, \dots}
\date{}
...
\abstract{
    ...
    We give a few examples of autocatalytic cycles occuring in central carbon metabolism, which are generally not considered as such, and for which our conclusions can be applied.
    This work thus serves as an approachable introduction to the analysis of autocatalytic metabolic cycles and as a demonstration of the insights one can get by such analysis.
}
...
While a few autocatalytic cycles are widely known, the definition we use for autocatalysis yields several autocatalytic cycles in central carbon metabolims that are not usually considered as such.

We identified 4 autocatalytic cycles in central carbon metabolism.
These cycles (presented in figure \ref{fig:realautocatal}) are:
(A) A cycle using phosphoenolpyruvate (PEP) to transport glucose using the phosphotransferase system (PTS), then converting the imported glucose into two PEP molecules;
(B) The glyoxylate cycle that can be used to produce two malate molecules from a single isocitrate molecule, assimilating acetyl-CoA, then converting the two malate molecules back into two isocitrate molecules \cite{Kornberg1966-lh};
(C) Various parts of the pentose phosphate cycle assimilating either erythrose-4P or sedoheptulose-7P (or both) to produce 5, 3 and 6 carbon compounds at higher stoichiometries;
(D) A cycle assimilating glycerone phosphate using the fructose bisphosphate aldolase reaction in the reverse direction, produceing fructose 1,6 bisphosphate from glyceraldehyde 3 phosphate, then converting the fructose back into two glyceraldehyde 3 phosphate using the pentose phosphate pathway.
Such examples suggest that the constraints we find on the kinetic parameters of enzymes involved in autocatalytic cycles may confine and shape the kinetic parameters of a broad set of enzymes central to metabolism.
...
\subsection{Input flux increases the range of parameters for which a stable steady state solution exists}
    ...
    For example, the CBB cycle can have input flux of fructose 6 phophate from the catabolism of starch.
...
A recent demonstration of these principles in vivo is the implementation of a functional CBB cycle in \emph{e.coli} by introducing the two genes missing for its functioning.
...
The examples of autocatalytic cycles we provide suggest that the constraints we find on the kinetic parameters of enzymes involved in autocatalytic cycles may confine and shape the kinetic parameters of a broad set of enzymes central to metabolism.

\section{Acknowledgments}
We would like to thank Elad Noor, Arren Bar-Even, Katja Tummler, Daniel Segre, Niv Antonovsky, Matthias Heinemann, David Fell and \dots for fruitful discussions and valuable insights contributing to this work.
